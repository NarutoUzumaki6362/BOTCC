...
def start(message):
    if str(message.chat.id) not in allowed_users:
        bot.reply_to(message, "🚫 𝐘𝐨𝐮 𝐜𝐚𝐧𝐧𝐨𝐭 𝐮𝐬𝐞 𝐭𝐡𝐞 𝐛𝐨𝐭 𝐭𝐨 𝐜𝐨𝐧𝐭𝐚𝐜𝐭 𝐝𝐞𝐯𝐞𝐥𝐨𝐩𝐞𝐫𝐬 𝐭𝐨 𝐩𝐮𝐫𝐜𝐡𝐚𝐬𝐞 𝐚 𝐛𝐨𝐭 𝐬𝐮𝐛𝐬𝐜𝐫𝐢𝐩𝐭𝐢𝐨𝐧 @SPIDY_OWNEROP")
        return
    bot.reply_to(message, "𝐒𝐞𝐧𝐝 𝐭𝐡𝐞 𝐭𝐱𝐭 𝐟𝐢𝐥𝐞 𝐧𝐨𝐰")

@bot.message_handler(commands=["add_user"])
def add_user(message):
    if str(message.chat.id) == '6710024903':  # Only bot owner can add new users
        try:
...
𝐁𝐨𝐭 𝐀𝐛𝐨𝐮𝐭: @SPIDY_OWNEROP 
𝐁𝐲: SPIDY_OWNEROP ''' #Declined
                    live += 1
                    bot.reply_to(message, msg)
                else:
                    dd += 1
